...
local function on_update()
    if ui.get(elements["Vehicles"]) then
        local vehicles = workspace_folder and workspace_folder:find_first_child("Vehicles")
        if vehicles then
            g_vehicles = {}
    ...
    if ui.get(elements["Landmarks"]) then
        local ignored = workspace_folder and workspace_folder:find_first_child("Ignored")
        if ignored then
            local shop = ignored:find_first_child("Shop")
            g_landmarks = {}
    
            for _, landmark_folder in pairs(shop:get_children()) do
                local main = landmark_folder:find_first_child("Head") or landmark_folder:find_first_child("ClickDetector")
                if main then
